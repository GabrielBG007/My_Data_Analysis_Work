...
dataX <- timeAverage(Calibración_5, avg.time= "hour")

dataS <- dataX[,-1]

 dataS[c(340:373,466:482),] <- NA

dataZ <- cbind(date=dataX$date, dataS)

data0 <- dataZ %>% select(date,"635 nm"=ScatRed3000, "525 nm"=ScatGreen3000, "450 nm"=ScatBlue3000)
...
p2 <- ggplot() +
  theme_bw() +
  geom_line(data=dataZ, aes(x=date, y= as.numeric(SAE3000), colour="SAE"),lwd=1, alpha=0.7) +
...
data4 <- dataZ %>% select(date, "635 nm"=fRHRed, "525 nm"=fRHGreen, "450 nm"=fRHBlue) %>%  pivot_longer(c("635 nm","525 nm","450 nm"), names_to="fRH3000", values_to="fRH3000V")
