...
p3 <- ggplot() +
  ...
  geom_smooth(data=data3, aes(x=date, y=SAE*1.1, colour= "SAE"), alpha=0.7, lwd=1, se=FALSE, span=0.17) +
  geom_point(data=data3, aes(x=date, y=SAE*1.1, fill= "SAE"), alpha=0.7, pch=21, colour="black", stroke=1) +
  ...
  scale_x_discrete("", limits=c(0,4,8,12,16,20)) +
  scale_y_continuous("fRH", sec.axis=sec_axis(trans~./1.1, "SAE(seco)")) +
  labs(y="fRH", x="") +
  theme(axis.title.y= element_text(size=16),
...
plot_grid(p1,p3,ncol=2,nrow=1, rel_widths=c(1,1), labels=c("a)","b)"))
