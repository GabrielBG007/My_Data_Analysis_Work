...
p3 <- ggplot() +
  ...
  annotate("text", x= 12.5, y= 156, label= expression("y=0.7205*PM"[10]*"- PM"[1]*"+ 3.5278*PM"[1]), colour="red") +
  annotate("text", x= 12.5, y= 146, label=expression("R"^2*"=0.9922 "), colour="red") +
  annotate("text", x= 22.5, y= 156, label=expression("y=0.2995*PM"[10]*"- PM"[1]*"+ 5.5517*PM"[1]) , colour="green") +
  annotate("text", x= 22.5, y= 146, label=expression("R"^2*"=0.9857 "), colour="green") +
  annotate("text", x= 32.5, y= 156, label= expression("y=-0.1486*PM"[10]*"- PM"[1]*"+ 7.9168*PM"[1]), colour="blue") +
  annotate("text", x= 32.5, y= 146, label=expression("R"^2*"=0.9804 "), colour="blue") 

title3 <- ggdraw() + draw_label("Días con intrusión de polvo", fontface="bold")

g3 <- plot_grid(title3, p3, ncol=1, rel_heights=c(0.1,1))
