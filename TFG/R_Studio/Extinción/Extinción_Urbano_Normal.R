...
data3 <- data2[-c(21:27,1,10:11,4:5,13,16),]
...
p2 <- ggplot() +
  ...
  annotate("text", x= 3, y= 78, label= expression("y=-0.0333*PM"[10]*"- PM"[1]*"+ 3.1479*PM"[1]), colour="red") +
  annotate("text", x= 3, y= 72, label=expression("R"^2*"=0.8138 "), colour="red") +
  annotate("text", x= 7, y= 78, label=expression("y=0.1739*PM"[10]*"- PM"[1]*"+ 4.3359*PM"[1]) , colour="green") +
  annotate("text", x= 7, y= 72, label=expression("R"^2*"=0.8165 "), colour="green") +
  annotate("text", x= 11, y= 78, label= expression("y=0.2892*PM"[10]*"- PM"[1]*"+ 5.5254*PM"[1]), colour="blue") +
  annotate("text", x= 11, y= 72, label=expression("R"^2*"=0.8069 "), colour="blue") 
