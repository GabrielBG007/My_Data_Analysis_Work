...
lmod1 <- lm(ScatRed3000~0 + PM10U_1, data1)
lmod2 <- lm(ScatGreen3000~0 + PM10U_1, data1)
lmod3 <- lm(ScatBlue3000~0 + PM10U_1, data1)
...
p <- data1 %>% 
  ...
  annotate("text", x=7.5, y=143, label= "y= 2.4617x", colour="red", size=3) +
  annotate("text", x=7.5, y=133, label= expression("R"^2*"=0.8233 "), colour="red", size=3) +
  annotate("text", x=12.5, y=143, label= "y= 3.1789x", colour="green", size=3) +
  annotate("text", x=12.5, y=133, label= expression("R"^2*"=0.7971  "), colour="green", size=3) +
  annotate("text", x=17.5, y=143, label= "y= 3.9729x", colour="blue", size=3) +
  annotate("text", x=17.5, y=133, label= expression("R"^2*"=0.7763 "), colour="blue", size=3) +
