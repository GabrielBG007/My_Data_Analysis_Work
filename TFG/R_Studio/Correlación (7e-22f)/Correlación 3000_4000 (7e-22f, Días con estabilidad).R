...
data1 <- Calibración[c(1422:1805,4131:4239,5258:5582),]
...
p1 <- ggplot() +
  ...
  annotate("text", x= 25, y= 365, label= "y=1.2089x", colour="red") +
  annotate("text", x= 25, y= 345, label=expression("R"^2*"=0.9237 "), colour="red") +
  annotate("text", x= 75, y= 365, label= "y=1.2092x", colour="green") +
  annotate("text", x= 75, y= 345, label=expression("R"^2*"=0.9582 "), colour="green") +
  annotate("text", x= 125, y= 365, label= "y=0.9884x", colour="blue") +
  annotate("text", x= 125, y= 345, label=expression("R"^2*"=0.9736 "), colour="blue") 

title1 <- ggdraw() + draw_label("Período 7 enero - 22 febrero (Días con gran estabilidad atmosférica)", fontface="bold")
