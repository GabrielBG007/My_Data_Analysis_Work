...
title5 <- ggdraw() + draw_label("Días con gran estabilidad atmosférica", fontface="bold")

x5 <- plot_grid(title5, p5,ncol=1, rel_heights=c(0.1,1))
 
 
 plot_grid(x1,x3,x5,ncol=3,nrow=1, rel_widths=c(1,1))
...
title6 <- ggdraw() + draw_label("Días con gran estabilidad atmosférica", fontface="bold")

 x6 <- plot_grid(title6, p6,ncol=1, rel_heights=c(0.1,1))

 d3 <- plot_grid(x5,x6, ncol=2, rel_widths=c(1,1))
 
 plot_grid(d1,d2,d3,ncol=1,nrow=3, rel_widths=c(1,1))
