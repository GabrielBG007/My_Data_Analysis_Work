...
title1 <- ggdraw() + draw_label("Días Normales(Mañana)", fontface="bold", size=8)
...
title2 <- ggdraw() + draw_label("Días Normales(Mañana)", fontface="bold", size=8)

 x2 <- plot_grid(title2, p2,ncol=1, rel_heights=c(0.1,1))

 plot_grid(x1,x2, ncol=2, rel_widths=c(1,1))
 ...
 title3 <- ggdraw() + draw_label("Días Normales(Noche)", fontface="bold", size=8)
 
   x3 <- plot_grid(title3, p3,ncol=1, rel_heights=c(0.1,1))
   
   
    g1 <- plot_grid(x1,x3,ncol=2,nrow=1,rel_widths=c(1,1))
  ...
  title4 <- ggdraw() + draw_label("Días Normales(Noche)", fontface="bold", size=8)
  
   x4 <- plot_grid(title4, p4,ncol=1, rel_heights=c(0.1,1))

   
   m1 <- plot_grid(x1,x2,x3,x4,ncol=2, nrow=2, rel_widths=c(1,1))
