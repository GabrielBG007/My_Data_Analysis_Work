...
p1 <- ggplot() + 
  ...
  title1 <- ggdraw() + draw_label("Días Normales (Mañana)", fontface="bold")
  
   m1 <- plot_grid(title1,p1,ncol=1,rel_heights=c(0.1,1))
...
#NOCTURNAS 
 ...
 title2 <- ggdraw() + draw_label("Días Normales (Noche)", fontface="bold")
 
  n1 <- plot_grid(title2,p2,ncol=1,rel_heights=c(0.1,1))
 ...
 q2 <- ggplot() + 
   theme_bw() +
   geom_line(data=data6, aes(x=hour, y=ScatRed4000, colour="635 nm", group=TRUE), lwd=1, alpha=0.7) +
   geom_point(data=data6, aes(x=hour, y=ScatRed4000, fill="635 nm", group=TRUE), pch=21, colour="black", stroke=1, size=2) +
   scale_colour_manual("", breaks= "635 nm", values= "red", guide=guide_legend(order=3)) +
   scale_fill_manual("", breaks= "635 nm", values= "red", guide=guide_legend(order=3)) +
   ggnewscale::new_scale_colour() +
   ggnewscale::new_scale_fill() +
   geom_line(data=data6, aes(x=hour, y=ScatGreen4000, colour="525 nm", group=TRUE), lwd=1, alpha=0.7) +
   geom_point(data=data6, aes(x=hour, y=ScatGreen4000, fill="525 nm", group=TRUE),pch=21, colour="black", stroke=1, size=2) +
   scale_colour_manual("", breaks= "525 nm", values= "green", guide=guide_legend(order=2)) +
   scale_fill_manual("", breaks= "525 nm", values= "green", guide=guide_legend(order=2)) +
   ggnewscale::new_scale_colour() +
   ggnewscale::new_scale_fill() +
   geom_line(data=data6, aes(x=hour, y=ScatBlue4000, colour="450 nm", group=TRUE), lwd=1, alpha=0.7) +
   geom_point(data=data6, aes(x=hour, y=ScatBlue4000, fill="450 nm", group=TRUE),pch=21, colour="black", stroke=1, size=2) +
   scale_colour_manual("", breaks= "450 nm", values= "blue", guide=guide_legend(order=1)) +
   scale_fill_manual("", breaks= "450 nm", values= "blue", guide=guide_legend(order=1)) + 
 ...
 title2 <- ggdraw() + draw_label("Días Normales (Noche)", fontface="bold")
  ...
  plot_grid(p2,q2, ncol=2, nrow=1, rel_widths=c(1,1))
 
 t2 <- plot_grid(title2,s2, ncol=1, rel_heights=c(0.1,1)) 

 x1 <- plot_grid(t1,t2,ncol=2,rel_widths=c(1,1))
