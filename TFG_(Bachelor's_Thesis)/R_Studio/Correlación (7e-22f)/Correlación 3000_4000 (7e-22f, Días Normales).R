...
data1 <- Calibración[-c(8191:10137,1:322,3188:3336,1422:1805,4131:4239,5258:5582,6445,6933),]
...
p1 <- ggplot() +
  ...
  annotate("text", x= 25, y= 365, label= "y=1.079x", colour="red") +
  annotate("text", x= 25, y= 345, label=expression("R"^2*"=0.8911 "), colour="red") +
  annotate("text", x= 75, y= 365, label= "y=1.1074x", colour="green") +
  annotate("text", x= 75, y= 345, label=expression("R"^2*"=0.9307 "), colour="green") +
  annotate("text", x= 125, y= 365, label= "y=1.0000x", colour="blue") +
  annotate("text", x= 125, y= 345, label=expression("R"^2*"=0.9493 "), colour="blue") 

title1 <- ggdraw() + draw_label("Período 7 enero - 22 febrero (Días Normales)", fontface="bold")
